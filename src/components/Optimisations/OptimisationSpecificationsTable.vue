    ...
    <div slot="actions" class="table-actions" slot-scope="actions, record">
      <a-dropdown :trigger="['click']">
        ...
        <a-menu slot="overlay">
          ...
          <a-menu-item>
            <a href="#" @click.prevent="deleteSpecification(record)"
...
export default {
  ...
  methods: {
    ...
    fetch(params = {}) {
      ...
      this.loading = true;
      axios.post(window.API_BASE + '/optimisations/' + this.optimisation.id + '/specifications/search', {
        results_per_page: 10,
        ...params
      }).then(r => {
    ...
    deleteSpecification(specification) {
      let vm = this;
      vm.isLoading = true;
      axios.delete(window.API_BASE + '/optimisations/' + this.optimisation.id + '/specifications/' + specification.id).then(() => {
        vm.isLoading = false;
    ...
    duplicateSpecification(specification) {
      let vm = this;
      vm.isLoading = true;
      axios.post(window.API_BASE + '/optimisations/' + this.optimisation.id + '/specifications/' + specification.id + '/duplicate').then(() => {
        vm.isLoading = false;
