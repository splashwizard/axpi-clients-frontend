            ...
            <a-col :span="12">
                <a-form layout="vertical">
                    ...
                    <a-form-item label="Shipping Address">
                        <a-select v-model="location.address_id"
                            ...
                            <a-select-option v-for="address in addresses"
                                             :value="address.id"
                                             :key="address.id">
                    ...
                    <a-form-item label="Ship Using">
                        <a-select v-model="location.ship_using"
                                  show-search size="large"
                                  @change="forceRefresh">
                            <a-select-option v-for="option in shipUsingOptions"
                                             :value="option.value"
                                             :key="option.value">
                                {{ option.label }}
                            </a-select-option>
                    ...
                    <a-form-item label="Shipping Method">
                        <a-select v-model="location.shipping_method"
                                  show-search size="large"
                                  @change="forceRefresh">
                            <a-select-option v-for="option in shippingMethodOptions"
                                             :value="option.value"
                                             :key="option.value">
                                {{ option.label }}
                            </a-select-option>
                    ...
                    <a-form-item label="Date Shipped">
                        <a-date-picker @change="forceRefresh"
                                       size="large" v-model="location.date_shipped"/>
                    </a-form-item>
                </a-form>
            </a-col>
        </a-row>
        <a-modal :centered="true" :width="700" :mask-closable="false"
                 v-model="createAddressModalVisible" title="Create Address" @ok="attemptSaveAddress">
