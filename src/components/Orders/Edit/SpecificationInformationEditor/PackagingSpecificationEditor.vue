<template>
  <div>
    ...
    <el-collapse v-model="activePanel" accordion>
      <el-collapse-item v-for="(component, i) in orderLocal.packaging_components" :key="i"
                        :title="getSectionNameLabel(component)" style="position: relative;">
        <a-dropdown style="position:absolute; right: 40px; top: 18px;" :trigger="['click']">
          <a-button type="link" icon="ellipsis"></a-button>
          <a-menu slot="overlay">
            <a-menu-item>
              <a href="#" v-if="orderLocal.packaging_components.length > 1" @click.prevent="deleteComponent(i)">Delete
                section</a>
            </a-menu-item>
          </a-menu>
        </a-dropdown>
        ...
        <div class="collapse-inner-section">
          ...
          <!-- Boxes -->
          <boxes-specification-editor @force-refresh="forceRefresh"
                                      :spec="orderLocal.packaging_components[i]" :order-local="orderLocal"
                                      v-if="component.type == 'box'"></boxes-specification-editor>
          <!-- / Boxes -->
