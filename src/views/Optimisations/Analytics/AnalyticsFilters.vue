      ...
      <div class="loaded" v-if="!isLoadingFilters">
        <div style="margin-bottom: 10px;">
          ...
          <div v-if="filter_by_supplier === true">
            <a-select style="width: 100%; margin-top: 15px; margin-bottom: 10px;"
                      v-model="selected_supplier_id"
                      show-search
                      placeholder="Select supplier">
              <a-select-option v-for="(supplier, i) in suppliers" :key="i"
        ...
        <div>
          ...
          <div v-if="filter_by_specification === true">
            <a-select style="width: 100%; margin-top: 15px; margin-bottom: 10px;"
              ...
              <a-select-option v-for="(specification, i) in optimisationSpecifications" :key="i"
                               :value="specification.id">{{ specification.product_name }}
              </a-select-option>
            </a-select>
          </div>
        </div>
...
export default {
  ...
  computed: {
    ...
    selected_specification_id: {
      ...
      set(id) {
        this.setSelectedSpecification(
            _.find(this.optimisationSpecifications, {id: id})
        );
      }
    }
  },
