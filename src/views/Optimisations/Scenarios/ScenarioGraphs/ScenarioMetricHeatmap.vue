  ...
  <div class="graph-container scenario-metric-heatmap" :key="updateKey">
    ...
    <div class="metric-selector-wrapper">
      <a-select style="width: 100%"
                v-model="selectedMetricIds" mode="multiple" @change="incrementUpdateKey">
  ...
  methods: {
    ...
    htmlContent(title, items) {
      ...
      for (var i = 0; i < items.length; i++) {
        ...
        var itemDom = '<li data-index={index}>' + '<span style="background-color:' + item.color + ';width:8px;height:8px;border-radius:50%;display:inline-block;margin-right:8px;"></span>' + item.name + '<span class="g2-tooltip-value">' + value + '</span>' + '</li>';
        listDom += itemDom;
      }
      listDom += '</ul>';
