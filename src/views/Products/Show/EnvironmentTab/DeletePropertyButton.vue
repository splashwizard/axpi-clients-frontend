    ...
    <template slot="content">
      <div class="popover-inner popconfirm">
        <a-button :disabled="isDeleting"
                  @click="visible = false" size="small" style="margin-right: 10px;">Cancel
        </a-button>
        <a-button :disabled="isDeleting"
                  @click.prevent="() => deleteProperty()" type="danger" size="small">Delete
        </a-button>
      </div>
    </template>
    <a-button :disabled="isDeleting"
  ...
  methods: {
    deleteProperty() {
      ...
      if (this.type === 'Materials') {
        axios.delete(window.API_BASE + '/products/' + this.productId + '/esg/materials/' + this.id).then(() => {
          vm.isDeleting = false;
      ...
      if (this.type === 'Certifications') {
        axios.delete(window.API_BASE + '/products/' + this.productId + '/esg/certifications/' + this.id).then(() => {
          vm.isDeleting = false;
      ...
      if (this.type === 'Transport' || this.type === 'Transportation') {
        axios.delete(window.API_BASE + '/products/' + this.productId + '/esg/transportations/' + this.id).then(() => {
          vm.isDeleting = false;
