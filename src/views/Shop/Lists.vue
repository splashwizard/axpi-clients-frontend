  ...
  <div class="shop-basket">
    <a-layout style="padding: 7px 30px">
      ...
      <a-card :bodyStyle="{padding: 0}">
        <lists-table :searchTerm="searchTerm" :lists="list" :updateVisible="updateVisible" :viewList="viewList" :handleEditList="handleEditList" :handleDeleteList="handleDeleteList"/>
      </a-card>
    </a-layout>
    <a-modal v-model="createListModalVisible" title="Create New List" @ok="handleCreateListModal">
      <a-form :form="createListForm" :label-col="{ span: 5 }" :wrapper-col="{ span: 12 }">
        <a-form-item label="List">
          <a-input
            v-decorator="['name', { rules: [{ required: true, message: 'Please input list name!' }] }]"
          />
        </a-form-item>
      </a-form>
    </a-modal>
    <a-modal v-model="editListModalVisible" title="Edit List" @ok="handleUpdateListModal">
      <a-form :label-col="{ span: 5 }" :wrapper-col="{ span: 12 }">
        <a-form-item label="List" :validate-status="editFormData.name.validateStatus" :help="editFormData.name.errorMsg">
          <a-input :value='editFormData.name.value' @change="(e) => handleEditListChange(e.target.value)"/>
        </a-form-item>
      </a-form>
    </a-modal>
    <a-modal v-model="deleteListModalVisible" title="Delete List" ok-text="Confirm" cancel-text="Cancel" @ok="handleDeleteListModal" :dialog-style="{maxWidth: '300px'}">
...
export default {
  ...
  methods: {
    ...
    updateVisible() {
      // const updateListIdx = this.lists.findIndex(list => list.id === id);
      // this.lists[updateListIdx].visible = !this.lists[updateListIdx].visible;
