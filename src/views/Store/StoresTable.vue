  ...
  >
    ...
    <div slot="actions" class="table-actions" slot-scope="status, store">
      <div v-if="status === 'requested'" class="d-flex">
        <a-button type="primary" :icon="approveLoading ? 'loading' : ''" @click="approveAccess(store)" class="mr-2" :disabled="approveLoading || approveLoading">Approve Access</a-button>
        <a-button type="danger" :icon="denyLoading ? 'loading' : ''" @click="denyAccess(store)" :disabled="approveLoading || approveLoading">Deny Access</a-button>
      </div>
...
export default {
  ...
  methods: {
    fetch() {
      this.loading = true;
      axios.get(window.API_BASE + '/stores').then(async (r) => {
        const stores = r.data;
    ...
    approveAccess(store) {
      ...
      axios.put(`${window.API_BASE}/supplier-store-links/${storeRequestId}`).then(() => {
    ...
    denyAccess(store) {
      ...
      const storeId = store.id;
      const storeRequestId = store.request_id;
      axios.delete(`${window.API_BASE}/supplier-store-links/${storeRequestId}`).then(() => {
        const storeIdx = this.data.findIndex(store => store.id === storeId);
