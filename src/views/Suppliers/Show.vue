<template>
  <div class="home">
    ...
    <a-page-header v-if="supplier"
        :title="supplier.name"
        @back="backToAllSuppliers"
    />

    <div class="page-body" v-if="supplier">

      <supplier-factory-details v-for="(factory, i) in supplier.factories" :factory="factory" :key="i"></supplier-factory-details>
...
export default {
  ...
  methods: {
    ...
    loadSupplier(id) {
      ...
      vm.order = null;
      vm.isLoading = true;
      axios.get(window.API_BASE + '/suppliers/' + id).then(r => {
        vm.isLoading = false;
        vm.supplier = r.data;
      }).catch(e => {
        vm.isLoading = false;
